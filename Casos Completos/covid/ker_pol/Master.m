...
[GAMMA,C,Ddis]=meshgrid(gamma_vec,c_vec,d_vec);
Bag_vals=[GAMMA(:),C(:),Ddis(:)]';
...
d_vec=Ddis(error_por(:)==min(error_por(:)))';
index_not=gamma_vec.*c_vec.*d_vec==min(gamma_vec.*c_vec.*d_vec); % nos  quedamos con el de menor peso cruzado
gamma_vec=gamma_vec(index_not);
c_vec=c_vec(index_not);
d_vec=d_vec(index_not);
...
Eqsys;
validator;
str=[name_data,'res_ker_pol',num2str(meta)];
save(str,'gamma','train_data','p','b','d','c');
fprintf("expected error is %f%%\n",error_por);