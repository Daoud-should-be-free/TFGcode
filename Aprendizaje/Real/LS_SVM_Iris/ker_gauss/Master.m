...
centering_sig=1/2;
sigma_vec=10.^((index_sig-centering_sig*dec_num_sig*bus_dec_sig)...
    ./bus_dec_sig);
...
index_not=gamma_vec.*sigma_vec==min(gamma_vec.*sigma_vec); % nos 
%quedamos con el de menor peso cruzado
...
generateKermatrix;%Sigma is now selected. I have noticed the system 
% is more robust against changes in sigma
%% Nit-pick gamma
...
for gamma_s=gamma_vec
    all_gammas(index,1:steps)=[
        linspace(gamma_s-gamma_s*log(10)/bus_dec,...
        gamma_s,steps/2),...
