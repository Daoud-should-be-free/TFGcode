...
dec_num_sig=7;%numero de decadas
index_sig=(1:bus_dec_sig*dec_num_sig);%#busquedas
centering_sig=1/8;
sigma_vec=10.^((index_sig-centering_sig*dec_num_sig*bus_dec_sig)./bus_dec_sig);
...
index_not=gamma_vec.*sigma_vec==min(gamma_vec.*sigma_vec); % nos  quedamos con el de menor peso cruzado
...
str=[name_data,'res_ker_gauss',num2str(max(s))];
save(str,'gamma','errors','error_por','p','b','sigma','time_elapsed','fake_neg','fake_pos');
